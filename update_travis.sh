...
EOF
# 生成 Server 构建参数
TAG_LATEST=$(curl --silent "https://api.github.com/repos/q3aql/aria2-static-builds/releases/latest" | grep tag_name | awk -F'"' '{print $4}')
TAGS=$(curl --silent "https://api.github.com/repos/q3aql/aria2-static-builds/releases" | grep tag_name | awk -F'"' '{print $4}')
for tag in ${TAGS}; do
    echo "    - DOCKER_REPO=benzbrake/aria2" >> .travis.yml.middle
    echo "      DOCKER_TAG=${tag}" >> .travis.yml.middle
    echo "      BUILD_DIRECTORY=./" >> .travis.yml.middle
